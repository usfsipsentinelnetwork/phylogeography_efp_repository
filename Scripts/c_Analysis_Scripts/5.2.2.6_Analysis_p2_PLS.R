...
RSS1 <- sum((predict(pls_out_1$model, type="response", comps=1) - to_analyze$Number.of.pests)^2)
RSS.m.clim <- sum((predict(pls_out_1$model, type="response", comps=1,
                           newdata = pls_out_1$model$dataX %>% mutate(Climate.Distance_centered=0)) - to_analyze$Number.of.pests)^2)
RSS.m.trade <- sum((predict(pls_out_1$model, type="response", comps=1,
                           newdata = pls_out_1$model$dataX %>% mutate(root4_trade_centered=0)) - to_analyze$Number.of.pests)^2)
RSS.m.fullint <- sum((predict(pls_out_1$model, type="response", comps=1,
                            newdata = pls_out_1$model$dataX %>% mutate(NMDS.Distance_centered=0,
                                                                       log_numexotics_centered=0,
                                                                       NMDS.Distance_centered.log_numexotics_centered=0)) - to_analyze$Number.of.pests)^2)
RSS.m.nmds <- sum((predict(pls_out_1$model, type="response", comps=1,
                              newdata = pls_out_1$model$dataX %>% mutate(NMDS.Distance_centered=0)) - to_analyze$Number.of.pests)^2)
RSS.m.trees <- sum((predict(pls_out_1$model, type="response", comps=1,
                           newdata = pls_out_1$model$dataX %>% mutate(log_numexotics_centered=0)) - to_analyze$Number.of.pests)^2)
RSS.m.int <- sum((predict(pls_out_1$model, type="response", comps=1,
                            newdata = pls_out_1$model$dataX %>% mutate(NMDS.Distance_centered.log_numexotics_centered=0)) - to_analyze$Number.of.pests)^2)

R2.all <- 1-RSS1 / TSS
R2.m.clim <- 1-RSS.m.clim / TSS
R2.m.trade <- 1-RSS.m.trade / TSS
R2.m.fullint<- 1-RSS.m.fullint / TSS
R2.m.nmds<- 1-RSS.m.nmds / TSS
R2.m.trees<- 1-RSS.m.trees / TSS
R2.m.int<- 1-RSS.m.int / TSS
...
save(
  ...
  pls_EU,
  pls_Au,
  file= "Final_output/PLS-models-9-10-23.RData"
)
