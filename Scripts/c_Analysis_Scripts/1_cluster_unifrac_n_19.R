...
bioregions_vector <- data.frame(melt(obj))%>%
...
save(bioregions_vector, file="Host_list/Output/unifrac_clusters_n_19.RData")

c19 <- #pals::polychrome(29)
  c(
    #"dodgerblue2"
    "green4",
    "#6A3D9A", # purple
...
clust19 <- NULL; for(i in 1:length(obj)) {
  zz19 <- rep(i,length(obj[[i]]))
  names(zz19) <- obj[[i]]
  clust19 <- c(clust19, zz19)
}
hcdendro19 <- as.dendrogram(hc.countries)
clust_reordered19 <- clust19[labels(hcdendro19)]

rotateorder19<-NULL
for (j in unique(clust_reordered19))
{rotateorder19<-c(
  rotateorder19,
  clust_reordered19[which(clust_reordered19==j)])}

names(c19) <- sort(names(obj))
c19a <- c19[names(obj)]


hcdendro19 <- as.dendrogram(hc.countries) %>%
  rotate(order=names(rotateorder19)) %>%
  set("labels_cex", .25) %>%
  set("labels_colors", c19a[rotateorder19]) %>%
  color_branches(
    col=c19a[rotateorder19] %>% unique,
    clusters=rotateorder19) %>%
  set("labels","") %>%
  set("branches_lwd", 1.25)

#hc.dendro.hclust19 <- as.hclust(hcdendro19)

#save(hc.dendro.hclust, file="Final_output/country_dendro_data_asplotted.RData")

gg.dendro.countries19<-as.ggdend(hcdendro19) %>% ggplot(horiz=F) + scale_y_sqrt()
windows(6,4);gg.dendro.countries19
save(gg.dendro.countries19, file="Figures_Sept2024/n19clusters.RData")
