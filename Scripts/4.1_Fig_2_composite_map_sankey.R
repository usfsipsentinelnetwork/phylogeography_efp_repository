...
load("Figures_Sept2024/1b_clusters_countries.RData")
...
sankeyplot_pathogens <-
  ...
  coord_flip(clip = "off") # add clip = "off" to prevent numbers from getting cut off
...
# added margins
panel3<- (p2 + theme(plot.margin = margin(0.5,0.5,0.5,0.5)) ) +
#  (sankeyplot_pathogens+ theme(plot.margin = margin(0.5,0.5,0.5,0.5))+ coord_cartesian(clip="off")) +
...
levels(world_map3$data$biogeographic_region) <-
  gsub("North America", "CONUS/Canada", levels(world_map3$data$biogeographic_region))

# storing for easy reproduction and modification
...
save(
  ...
  sink_map_EU,
  sink_map_AU,
  file = "Figures_Sept2024/Fig2_components.RData"
)
...
save(composite_figure2, file='Figures_Sept2024/Fig2_composite_ABCD.RData')

#load('Figures_Sept2024/Fig2_composite_ABCD.RData')

library(devEMF)

composite_figure2

emf(file = "Final_figures/Fig2_composite_ABCD.emf", 6,7, emfPlus=T)
composite_figure2
dev.off()

svg("Final_figures/Fig2_composite_ABCD.svg", 6,7, family="Cambria Math")
composite_figure2
dev.off()
