...
library(phyloseq)
library(WorldFlora)
source("Scripts/Script1_Sentinels_functions.R")
...
#
#names_new <- master_names_list_hosts_all[which(!(master_names_list_hosts_all %in% unique(working_trans_table$spec.name.ORIG)))]
...
for (i in 0:(length(names_new) %/% chunksize)) {
  start <- i*chunksize + 1

  if (i*chunksize == length(names_new)) break
  if (i == length(names_new) %/% chunksize) {stop <- start + length(names_new) %% chunksize} else {stop <- start + chunksize-1}
  
  if (((start - 1) %% chunksize) == 0)
    print(paste("Processing entries", start, "to", stop))
  
  xx<-WFO.match(spec.data=names_new[start:stop], WFO.data=WFO.data, Fuzzy=0.0, squish=T)
  ...
  print(paste("Wrote entries 1-", stop, "to file!"))
}
