...
#binomial_or_greater<-grepl("ae ", names_new)

names_new <- names_new[-which(names_new == "Labiatae")]
...
lists <- foreach(a=h, .combine=rbind, .packages=c('tidyr', 'WorldFlora')) %dopar% {
  ...
  yy<-WFO.one(xx)
  filename <- paste("Host_list/Output/TranslationTable_WFO/", "names_", start+missing_number, stop+missing_number, sep="")
...
for (y in h) {
  ...
  filenames_all <- rbind(filenames_all, data.frame(name=paste("Host_list/Output/TranslationTable_WFO/", "names_", start+missing_number, stop+missing_number, sep="")))
}

missing <- setdiff(filenames_all$name, lists)
