...
#load("Final_output/Final_analysis_data_pathogens-March1-2025-pnas.RData")
load("Final_output/Final_analysis_data_pathogens-March4-2025-pnas.RData")

hist(to_analyze$NMDS.Distance)
...
# need to remake this file
#save(to_analyze,to_analyze_zeroadjusted, file="Final_output/Final_analysis_data_pathogens-transformations-March1-2025-pnas.RData")
save(to_analyze,to_analyze_zeroadjusted, file="Final_output/Final_analysis_data_pathogens-transformations-March4-2025-pnas.RData")
...
fig3_correlations_plot<-
  ...
  facet_wrap(~ variable, ncol=2, nrow=2, scales="free_x",
             labeller=as_labeller(c(root4_trade="Imports (Trillion USD) [∜ scale]",#"Imports (∜ Trillion USD)",
                                    ...
                                    log_numexotics="Established Trees & Shrubs [log scale]")),
...
to_analyze$NMDS.Distance%>% hist

fig3_correlations_plot<-
  fig3_correlations_plot +
  facetted_pos_scales(
    x = list(
      scale_x_continuous(limits = c(0.1,.65), breaks = c(0,.2,.4,.6)),
      scale_x_continuous(breaks = function(x) seq(x[1],x[2], length.out=5), labels = function(x) round(x^4,1)),
      scale_x_continuous(limits = c(0.1,1), breaks = c(.2,.4,.6,.8,1)),
...
save(fig3_correlations_plot, file="Figures_Sept2024/fig4a_correlations_plot_march4_2025.RData")

svg("Final_figures/Fig4_A_march4_2025.svg", 9.5,6.5,family="Cambria Math")
fig3_correlations_plot
dev.off()
...
scale_climate_x <- function() scale_x_continuous(limits = c(0.1,.65), breaks = c(0,.2,.4,.6))
...
scale_nmds_x <- function() scale_x_continuous(limits = c(0.1,1), breaks = c(.2,.4,.6,.8,1))
scale_trade_y <- function() scale_y_continuous(#limits = c(0,1),
                                               breaks = function(x) seq(x[1],x[2], length.out=5), labels = function(x) round(x^4,1))
scale_nmds_y <- function() scale_y_continuous(limits = c(0.1,1), breaks = c(.2,.4,.6,.8,1))
...
fig3b_correlations <-
  egg::ggarrange(
    ...
    (ggplot(to_analyze,aes(x=Climate.Distance, y=log_numexotics, color=Geographic.Origin.Cluster, pch=Invaded.Range.Cluster)) %>% corrplot_panel(left="Established\nTrees & Shrubs\n[log scale]", bottom="\nClimatic\nDistance") + scale_climate_x() + scale_trees_y())%>%
...
save(fig3b_correlations, file="Figures_Sept2024/fig4b_colinearity_plot_march4_2025.RData")

svg("Final_figures/Fig4_B_march4_2025.svg", 9.5,6.5,family="Cambria Math")
fig3b_correlations
dev.off()
...
save(fig3_composite, file="Figures_Sept2024/fig4_composite_march4_2025.RData")

svg("Final_figures/Fig4_AB_march4_2025.svg", 7,10,family="Cambria Math")
fig3_composite
dev.off()

emf(file = "Final_figures/Fig4_AB_march4_2025.emf", 7,10, emfPlus=T)
fig3_composite
dev.off()
